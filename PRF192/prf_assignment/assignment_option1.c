...
void modifyProducts(const char *filename) {
    ...
    while(fread(&prod, sizeof(struct Product), 1 ,fp) == 1) {
        if(prod.product_id == targetID) {
            ...
            // `ftell(fp)` returns the current position in the file (in bytes), showing how far into the file the program is.
            pos = ftell(fp) - sizeof(struct Product);
            ...
            scanf("%d", &prod.quantity);
            /*  
                It moves the "cursor" in the file to a specific position (pos bytes from the start), so reading or writing begins there.
