...
public class DeleteController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("delete".equals(fromPage)) url += "&fromPage=delete";

        try {
            HttpSession session = request.getSession(false);
            if (session != null) {
                UserDto user = (UserDto) session.getAttribute("USER_INFO");
                if (user != null && "ST".equals(user.getRoleID())) {
                    url = "MainController?btAction=Search&txtSearchValue=" + lastSearchValue + "&ERROR=No_Permission";
                } else {
                    if (itemID != null) {
                        ComponentDao dao = new ComponentDao();
                        dao.deleteComponent(itemID);
                    }
