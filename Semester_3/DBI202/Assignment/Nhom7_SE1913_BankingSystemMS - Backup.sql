...
********************************************************************************/
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'BankingSystemMS')
BEGIN
	ALTER DATABASE BankingSystemMS SET OFFLINE WITH ROLLBACK IMMEDIATE;
	ALTER DATABASE BankingSystemMS SET ONLINE;
	DROP DATABASE BankingSystemMS;
END

GO

CREATE DATABASE BankingSystemMS
GO
