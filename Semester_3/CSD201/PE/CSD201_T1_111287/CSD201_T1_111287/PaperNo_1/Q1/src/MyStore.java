...
class requestQueue {
    ...
    void enQueue(int size, int price) {
        ...
        Room newRoom = new Room(size, price);
        Node newNode = new Node(newRoom);
        if (size > 0 && price > 0) {
...
//===========================================================================
    ...
    void rent(Room t) throws Exception {
        ...
        while(current != null) {
            if(current.info.getStatus() == 0 && current.info.getSize() >= t.getSize() && current.info.getPrice() <= t.getPrice()) {
                if (bestRoom == null || current.info.getPrice() < bestRoom.getPrice()) bestRoom = current.info;
            }   
            current = current.next;
        }
        
        if (bestRoom != null) bestRoom.setStatus(1);
    ...
    void f4() throws Exception {
        ...
        Node current = dList.head;
        while (current != null) {
            if (current.info.getStatus() == 0) count++;
            current = current.next;
        }
