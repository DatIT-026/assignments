...
public void bfs(Node p) { // input root
    ...
    while (!m.isEmpty()) {
        ...
        if (q.left != null) m.enqueue(q.left);  // if lever still have a node left
        if (q.right != null) m.enqueue(q.right); // if lever still have a node right
...
public void insert(Car x) {
    ...
    while (p != null) {
        ...
        f = p;
        if (x.price < p.info.price) p = p.left;
        else p = p.right;
    }
    if (x.price < f.info.price) f.left = q;
    else f.right = q;
}
...
public Node search(Node p, Person x) {
    ...
    if (p.info.name.equals.x.name) return p;
    else if (p.info.name.compareToIgnoreCase(x.name) > 0) return search(p.left, x);
...
public void deleByCopy(int xPrice) {
    ...
    if (p.left == null && p.right == null) { // p is a leaf node
        if (f == null) root = null; // The tree is one node
        else {
    ...
    if (p.left != null && p.right == null) { // p has only left child
        if (f == null) root = p.left; // p is a root
            ...
            if (f.left == p) f.left = p.left; // p is a left child
            else f.right = p.left;
        }
        return;
    }
    ...
    if (p.left == null && p.right != null) { // p has only right child
        if (f == null) root = p.right; // p is a root
            ...
            if (f.left == p) f.left = p.right; // p is aleft child
    ...
    if (p.left != null && p.right != null) { // p has both left and right children
        ...
        if (fr == null) p.left = rp.left; // rp is just a left son of p
        else fr.right = rp.left;
    }
}
...
void deleByCopy(String xName) {
    ...
    while (p != null) {
        ...
        f = p;
        if (xName.compareTo(p.info.name) < 0) p = p.left;
        else p = p.right;
    }
    ...
    if (p.left != null && p.right != null) { // find the right most node
        ...
        if (frp == null) p.left = q.left; // rp = q
...
public void deleteByCopy(Node p) {
    ...
    // 2. p has a left child only
        ...
        if (f == null) root = p.left; // remove root
    ...
    // 3. p has a right child only
        ...
        if (f == null) root = p.right; // remove root    
    ...
    // 4. Both of right and left child
        ...
        p.info = rp.info;
        if (f == null) p.left = rp.left; // rp has no right child
...
public void calculateBalance(Node p) {
    ...
    while (!m.isEmpty()) {
        ...
        if (isAVL && q.bal < -1 || q.bal > 1) isAVL = false; // De cho thuc hien nhieu
    }
}
...
void fvisitBal(Node p, RandomAccessFile f) throws Exception {
    if (p != null) f.writeBytes("(" + p.info.name + "," + p.info.age + "," + p.bal + ") ");
}
...
void fvisitLevel(Node p, RandomAccessFile f) throws Exception {
    if (p != null) f.writeBytes("(" + p.info.name + "," + p.info.age + "," + p.level + ") ");
}
