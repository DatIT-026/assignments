...
public class MyList {
    ...
    void addLast(int info) {
        if (isEmpty()) head = tail = new Node(info);
        else {
    ...
    void delLast() {
        // ham nay xu li truong hop neu danh sach rong -> khong co gi de xoa
        if (head == null) return;

        // neu danh sach chi co MOT nut (head la nut cuoi)
        // xoa nut cuoi cung~ giong nhu xoa nut dau
        if (head.next == null) {
            head = null;
            tail = null;
            return;
        ...
        // neu danh sach co 2 nut tro len
        Node node = head;
